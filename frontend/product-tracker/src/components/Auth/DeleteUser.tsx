...
export default function DeleteUser({ id, onDeleted, onApiError }: DeleteUserProps){
    ...
    const handleClick = (id: String) => {
        const userRes = confirm('¿Desea eliminar el usuario?')

        if(userRes){
            const deleted = deleteApiUser(id)
        }   
    }
    
    return (
        <svg  onClick={() => handleClick(id)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
